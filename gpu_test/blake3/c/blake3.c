...
void blake3_hasher_finalize_seek(const blake3_hasher *self, uint64_t seek,
  ...
  if (self->cv_stack_len == 0) {
    ...
    #ifdef DEBUG_CPU
      /* -------- Debug print: root CV words 0‑3 ---------------------- */
      {
        uint32_t cv_words[8];
        memcpy(cv_words, output.input_cv, 32);
        blake3_compress_in_place(cv_words,
                                 output.block,
                                 output.block_len,
                                 output.counter,
                                 output.flags | ROOT);
        printf("CPU root cv[0..3]   = %08X %08X %08X %08X\n",
               cv_words[0], cv_words[1], cv_words[2], cv_words[3]);

        /* ---- Debug print: first 64‑B XOF block (words 0‑3) ---------- */
        uint8_t zero_block[64] = {0};
        ((uint32_t *)zero_block)[15] = DERIVE_KEY_MATERIAL;
        uint8_t wide[64];
        blake3_compress_xof(cv_words,
                            zero_block,
                            64,                 /* block length          */
                            0,                  /* output‑block counter  */
                            DERIVE_KEY_MATERIAL,
                            wide);
        printf("CPU XOF blk0[0..3] = %08X %08X %08X %08X\n",
               ((uint32_t*)wide)[0], ((uint32_t*)wide)[1],
               ((uint32_t*)wide)[2], ((uint32_t*)wide)[3]);
      }
    #endif
